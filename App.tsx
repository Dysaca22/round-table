...
import { PARTICIPANTS as defaultParticipants, DEBATE_TOPIC as defaultTopic } from './constants';
import { startDebate, getMemberContribution, getModeratorDecision } from './services/geminiService';
...
const App: React.FC = () => {
    ...
    const [debateStatus, setDebateStatus] = useState<DebateStatus>(DebateStatus.SETTINGS);
    const [currentTurn, setCurrentTurn] = useState<Turn | null>(null);
    const [statusText, setStatusText] = useState("");
    const [error, setError] = useState<string | null>(null);

    // Configurable state
    const [participants, setParticipants] = useLocalStorage<Participant[]>('participants', defaultParticipants);
    const [debateTopic, setDebateTopic] = useLocalStorage<string>('debateTopic', defaultTopic);
    ...
    const [thinkingTime, setThinkingTime] = useLocalStorage<number>('thinkingTime', 4);
    ...
    const addMessage = (participantId: string, text: string) => {
        setMessages(prev => [{ id: crypto.randomUUID(), participantId, text }, ...prev]);
    };
    ...
    const handleStartDebate = () => {
        ...
        startDebate(debateTopic, participants, language, aiConfig)
            .then(({ contribution, nextSpeakerId }) => {
                ...
                setCurrentTurn({ participantId: nextSpeakerId, action: 'CONTRIBUTING' });
            })
            .catch(err => {
                console.error(err);
    ...
    const processTurn = useCallback(async () => {
        ...
        turnCount.current++;
        setStatusText(currentTurn.action === 'CONTRIBUTING'
        ...
        try {
            ...
            if (currentTurn.action === 'CONTRIBUTING') {
                const { contribution } = await getMemberContribution(currentParticipant, messages, participants, debateTopic, language, aiConfig);
                addMessage(currentParticipant.id, contribution);
                setCurrentTurn({ participantId: 'moderator', action: 'DECIDING' });
            } else if (currentTurn.action === 'DECIDING') {
                const moderator = participants.find(p => p.role === ParticipantRole.MODERATOR)!;
                const { contribution, nextSpeakerId } = await getModeratorDecision(moderator, messages, participants, debateTopic, language, aiConfig);
    ...
    }, [currentTurn, debateStatus, messages, participants, debateTopic, language, aiConfig, t, thinkingTime]);
    ...
    const handleExportDebate = () => {
        ...
        const transcript = messages
            ...
            .map(msg => {
                const participantName = participantsMap.get(msg.participantId) || 'Unknown';
    ...
    return (
      <div className="bg-slate-900 text-slate-300 min-h-screen flex font-sans">
        ...
        <aside className="w-80 flex-shrink-0 bg-slate-950 p-6 hidden lg:flex flex-col space-y-6">
           <div>
             <h1 className="text-2xl font-bold text-white mb-1">{t.app.title}</h1>
        ...
        <main className="flex-1 flex flex-col h-screen max-w-full overflow-hidden">
            <header className="p-4 border-b border-slate-800 bg-slate-950/50 backdrop-blur-sm flex-shrink-0 flex items-center justify-between gap-4">
                <p className="text-slate-300 text-center truncate flex-grow" title={debateTopic}>{debateTopic}</p>
                <button 
            ...
            <div className="flex-grow flex flex-col overflow-y-auto">
              <div className="mt-auto"> {/* Aligns messages to the bottom initially */}
                  {messages.slice().reverse().map((msg) => (
                    <ChatBubble key={msg.id} message={msg} participant={participants.find(p => p.id === msg.participantId)} />
                  ))}
              </div>
            </div>
            ...
            <footer className="border-t border-slate-800 bg-slate-950/80 backdrop-blur-sm p-3 flex-shrink-0">
                {error && <p className="text-red-400 mb-2 text-center text-sm px-4">{error}</p>}
                
                <div className="flex items-center justify-between gap-4 w-full max-w-4xl mx-auto">
                    {debateStatus === DebateStatus.RUNNING || debateStatus === DebateStatus.PAUSED ? (
                        <>
                           <button onClick={togglePause} className="flex items-center justify-center h-10 w-10 rounded-full bg-slate-700 hover:bg-slate-600 text-slate-200 transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500">
                        ...
                        <div className="w-full flex flex-col items-center">
                            <p className="text-slate-400 mb-2">{statusText}</p>
                             <button onClick={() => setDebateStatus(DebateStatus.SETTINGS)} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-6 rounded-md transition-colors">
